"""Test suite for server performance with 250k strings.

This module benchmarks server performance with a dataset of 250k strings.
Typical execution times:
- Without reread: ~0.15-0.35ms per request
- With reread: ~25-35ms per request

The test measures:
- Response times with reread enabled/disabled
- Performance with 250k string dataset
...
def generate_250k_strings_file() -> str:
    """Generate a temporary file with 250,000 unique strings.
    
    Returns:
        str: Path to the temporary file containing 250k strings
    """
    temp_file = tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False)
    
    # Generate 250k unique strings
    for i in range(250_000):
        temp_file.write(f"string_{i:06d}\n")
...
def benchmark_server_performance(server_instance: Server, num_requests: int = 100) -> None:
    ...
    """
    # Generate temp file with 250k strings
    temp_file_path: str = generate_250k_strings_file()
    ...
    try:
        ...
        print(f"\nPerformance Results (250k strings):")
        ...
        assert avg_no_reread <= 0.4, f"Performance without reread exceeded 0.4ms: {avg_no_reread:.3f}ms"
        assert avg_with_reread <= 38.0, f"Performance with reread exceeded 38ms: {avg_with_reread:.3f}ms"
