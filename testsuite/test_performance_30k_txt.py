"""Test suite for server performance with 30k strings.

This module benchmarks server performance with a dataset of 30k strings.
Typical execution times:
- Without reread: ~0.08-0.2ms per request
- With reread: ~20-30ms per request

The test measures:
- Response times with reread enabled/disabled
- Performance with 30k string dataset
...
def generate_30k_strings_file() -> str:
    """Generate a temporary file with 30,000 unique strings.
    
    Returns:
        str: Path to the temporary file containing 30k strings
    """
    temp_file = tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False)
    
    # Generate 30k unique strings
...
def benchmark_server_performance(server_instance: Server, num_requests: int = 100) -> None:
    ...
    """
    # Generate temp file with 30k strings
    temp_file_path: str = generate_30k_strings_file()
    ...
    try:
        ...
        print(f"\nPerformance Results (30k strings):")
        ...
        assert avg_no_reread <= 0.3, f"Performance without reread exceeded 0.3ms: {avg_no_reread:.3f}ms"
        assert avg_with_reread <= 35.0, f"Performance with reread exceeded 35ms: {avg_with_reread:.3f}ms"
