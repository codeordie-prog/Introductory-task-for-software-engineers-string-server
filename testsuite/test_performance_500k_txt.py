"""Test suite for server performance with 500k strings.

This module benchmarks server performance with a dataset of 500k strings.
Typical execution times:
- Without reread: ~0.2-0.4ms per request
- With reread: ~30-40ms per request

The test measures:
- Response times with reread enabled/disabled
- Performance with 500k string dataset
...
def generate_500k_strings_file() -> str:
    """Generate a temporary file with 500,000 unique strings.
    
    Returns:
        str: Path to the temporary file containing 500k strings
    """
    temp_file = tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False)
    
    # Generate 500k unique strings
...
def benchmark_server_performance(server_instance: Server, num_requests: int = 100) -> None:
    ...
    """
    # Generate temp file with 500k strings
    temp_file_path: str = generate_500k_strings_file()
    ...
    try:
        ...
        print(f"\nPerformance Results (500k strings):")
        ...
        assert avg_no_reread <= 0.5, f"Performance without reread exceeded 0.5ms: {avg_no_reread:.3f}ms"
        assert avg_with_reread <= 42.0, f"Performance with reread exceeded 42ms: {avg_with_reread:.3f}ms"
